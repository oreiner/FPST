...
%========================================================
function [output] = VAS_horz(subjectID, studyID, sessionID, questionlist, window, screen_config, preset, out_ind, trial_i)
    %% Preparation
    %{
    if preset ~= 1
        ...
        [window,screenrect] = Screen('OpenWindow',screen_config.screenNumber,[0 0 0],[0 0 800 600]);
    ...
    for i=1:length(questionlist{1,1}); %number of questions
        ...
        trial.question = questionlist{1,1}{i};
     ...
     if  strcmp(trial.question,'wollen')
    text_question = 'Würden Sie die Lösung nach dem Experiment trinken wollen?';
        Screen('TextSize',rating_scr,16);
        Screen('TextFont',rating_scr,'Arial');
            Screen('TextFont',rating_scr,'Arial');
        anchor_1 = ['Ich möchte \n die Lösung auf gar keinen Fall trinken '];
        %anchor_4 = ['Neutral'];
        anchor_7 = ['Ich möchte die Lösung \n sehr gerne trinken']; 

        else text_question = ['Wie ' text_freerating ' ist die Lösung?'];
         Screen('TextSize',rating_scr,16);
        Screen('TextFont',rating_scr,'Arial');
        anchor_1 = ['absolut nicht ' trial.question];
        %anchor_4 = ['Neutral'];
        anchor_7 = ['sehr ' trial.question];   
        ...
        DrawFormattedText(rating_scr, text_question, 'center', (Scale_offset+Screen_height/2 - 100), [205 201 201],40,[],[],2);
        ...
        while mousebuttons(1)~=1 
            ...
            if (GetSecs > nextTime) %Sample every 0.01 seconds
                ...
                if mousebuttons(1)==1 %Terminate and record rating on left mouseclick
                    output.rating(trial_i,out_ind) = ((Slider_x_pos - (Screen_width/2-Scale_width/2))/ Scale_width)*100; %rescaling of scale_width independent of screen resolution [0-100]
