...
function [output] = FPST_Ratings(subjectID, studyID, sessionID, sess_type, screen_config, window, simulation)
    ...
    % Setup
    screen_config.feedback_delay = 1; %specifies the duration that the confirmed rating will be displayed on the screen
    ...
    screen_config.Screen_width = screen_config.screenrect(3)-screen_config.screenrect(1);
    screen_config.Screen_height = screen_config.screenrect(4)-screen_config.screenrect(2);
    ...
    if any(strcmp(sess_type, 'gLMS_Training'))
        ...
        Screen_width = screen_config.screenrect(3)-screen_config.screenrect(1);
        Screen_height = screen_config.screenrect(4)-screen_config.screenrect(2);
        Scale_width = 0;
    ...
    elseif any(strcmp(sess_type, 'Internal_State_1')) || any(strcmp(sess_type, 'Internal_State_2'))
        ...
        state_ind = 1;
        state_out_ind = 1;
        f_VAS_horz_state(subjectID, studyID, sessionID, statelist, window, screen_config, preset ,state_ind, state_out_ind);
        output.time = datestr(now);
        onset_flavor_rating = GetSecs;
        ...
        state_ind = 1;
        state_out_ind = 1;
        [output.state] = VAS_horz_state(subjectID, studyID, sessionID, statelist, window, screen_config, preset ,state_ind, state_out_ind);
        ...
        for trial_i=1:num_trials
            ...
            [output.rating1] = LMS_vertical(subjectID, studyID, sessionID, sensationslist, window, screen_config, preset , out_ind, trial_i); %script to run labeled magnitude scales
            [output.rating2] = LHS_vertical(subjectID, studyID, sessionID, questionlist, window, screen_config, preset , out_ind, trial_i); %script to run labaled hedonic scales
            [output.rating3] = VAS_horz(subjectID, studyID, sessionID, questionlist, window, screen_config, preset , out_ind, trial_i); %script to run visual analogue scales
            output.rating = [output.rating1 ; output.rating2; output.rating3]; 
