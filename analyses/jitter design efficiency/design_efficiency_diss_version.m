...
spm_jobman('initcfg');
numtrial=46*6;
list = 'with_most_important_onlyshowncontrasts_500_corrected'; %gives the run a unique name
dummy_volumes = 10;
knum=500;
...
for k=1:knum
    ...
    elseif iJitter == 5
        % exp with more time
        %total=5.5min
        jitter(k).u(1,:)=0.9+exprnd(0.3,numtrial,1);    
    elseif iJitter == 6
        % total = 6.8min
        jitter(k).u(1,:)=1.2+exprnd(0.3,numtrial,1); 
    elseif iJitter == 7
        %time=8.1min
        jitter(k).u(1,:)=1.2+exprnd(0.6,numtrial,1); 
    elseif iJitter == 8
        %time=9.7min
        jitter(k).u(1,:)=1.5+exprnd(0.6,numtrial,1); 
    elseif iJitter == 9
        %time=13min
        jitter(k).u(1,:)=2+exprnd(0.9,numtrial,1);      
    elseif iJitter == 10
        %two exp jitters
    ...
    elseif iJitter == 12
     ...
     elseif iJitter == 13
         %two exp jitters
         %meanjitt=3.2s total_both=8.1min (+3.5min)
         jitter(k).u(1,:)=0.6+exprnd(0.3,numtrial,1);
         jitter(k).u(2,:)=0.6+exprnd(0.3,numtrial,1);
        response_time=repmat(1.7,1,numtrial);
    elseif iJitter == 14
        %total time= 11min
        jitter(k).u(1,:)=0.9+exprnd(0.3,numtrial,1);
         jitter(k).u(2,:)=0.9+exprnd(0.3,numtrial,1);
    ...
    elseif iJitter == 16
        %total time = 13min
       jitter(k).u(1,:)=1.2+exprnd(0.3,numtrial,1);
         jitter(k).u(2,:)=1.2+exprnd(0.3,numtrial,1);
       
        elseif iJitter == 17
            %total time = 13min + (3.5min)
       jitter(k).u(1,:)=1.2+exprnd(0.3,numtrial,1);
         jitter(k).u(2,:)=1.2+exprnd(0.3,numtrial,1);
        response_time=repmat(1.7,1,numtrial);
    end
