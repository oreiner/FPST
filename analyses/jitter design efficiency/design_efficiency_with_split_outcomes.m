...
spm_jobman('initcfg');
numtrial=46*6;
list = 'with_second_jitter'; %gives the run a unique name
dummy_volumes = 10;
...
for k=1:500
    ...
    if iJitter == 1
        ...
        %{
        %I don't want to have 50 repeatitions of each subject, but having less messes up the code for some reasin 
        if k>numel(subjects);
            k=500;
            break;
        end
    ...
    elseif iJitter == 4
        % exp with more time
        % jitt1 mean=1.5s; total= 7.3min
        jitter(k).u(1,:)=1.0+exprnd(0.6,numtrial,1);
    elseif iJitter == 5
        % exp with more time
        % %meanjitt=3.2s %total=14.2min
        jitter(k).u(1,:)=1.9+exprnd(1.3,numtrial,1);    
    elseif iJitter == 6
        % UNIFORM
        % jitt1 mean=0.7s; total=3.4min
    ...
    elseif iJitter == 7
        ...
        jitter(k).u(1,:)=round((jitter(k).u(1,:)/1000),1)';
    elseif iJitter == 8
        % UNIFORM with more time
        % jitt1 mean=1.45s; total=6.7min
        jitter(k).u(1,:)=randi([0 3000],1,numtrial);
        jitter(k).u(1,:)=round((jitter(k).u(1,:)/1000),1)';
    elseif iJitter == 9
        %mean=1.5s; total=6.9min
        x = [0,500,1000,2000,2500,3000];
        y = repmat(x,1,numtrial/numel(x));
        jitter(k).u(1,:)=y(randperm(numel(y)));
        jitter(k).u(1,:)=round((jitter(k).u(1,:)/1000),1)';     
    elseif iJitter == 10
        %two exp jitters
        %jitt1 mean =0.45s; total1= 2.1; total_both=4.2min
         jitter(k).u(1,:)=0.3+exprnd(0.15,numtrial,1);
         jitter(k).u(2,:)=0.3+exprnd(0.15,numtrial,1);
     elseif iJitter == 11
        %two exp jitters with max response time
        %jitt1 mean =0.45s; total1= 2.1; total_both=4.2min 
         jitter(k).u(1,:)=0.3+exprnd(0.15,numtrial,1);
         jitter(k).u(2,:)=0.3+exprnd(0.15,numtrial,1);
         response_time=repmat(1.7,1,numtrial);
    elseif iJitter == 12
        %two exp jitters
        % jitt1 mean=0.9s; total=4.06min; total both=8.1min
         jitter(k).u(1,:)=0.6+exprnd(0.3,numtrial,1);
         jitter(k).u(2,:)=0.6+exprnd(0.3,numtrial,1);
     elseif iJitter == 13
         %two exp jitters
         %meanjitt=3.2s total_both=29 min
         jitter(k).u(1,:)=1.9+exprnd(1.3,numtrial,1);
         jitter(k).u(2,:)=1.9+exprnd(1.3,numtrial,1);
        
    elseif iJitter == 14
        % two UNIFORM
        % jitt1 mean=0.35s; total1=1.7min; total_both= 3.2min
    ...
    elseif iJitter == 15
        % two UNIFORM
        % jitt1 mean=1; 
        % jitt1 mean=0.7s; total=3.4min; both=6.8min
        jitter(k).u(1,:)=randi([0 1500],1,numtrial);
        jitter(k).u(1,:)=round((jitter(k).u(1,:)/1000),1)';
    ...
    elseif iJitter == 16
        ...
        elseif iJitter == 17
         % 0 or 500 or 1000 or  1500 
        % jitt1 mean=3s; jitt2 mean=3s; both=13.8+13.8min
        x = [1000,2000,3500,5500];
        y = repmat(x,1,numtrial/numel(x));
        jitter(k).u(1,:)=y(randperm(numel(y)));
        jitter(k).u(1,:)=round((jitter(k).u(1,:)/1000),1)';
        jitter(k).u(2,:)=y(randperm(numel(y)));
   ...
   %?
    ...
    [simulation] = FPST_simulate_onsets(jitter(k).u, end_results.trial, posterior.PE, response_time);
