function [status,output] = FPST_call_fsl(cmd)
...
% indicated
%setenv('FSLDIR','/usr/share/fsl/5.0/etc/fslconf');
setenv('FSLDIR','/usr/local/FSL/fsl/');
...
if status
    error('FSL call (%s) failed, %s', cmd, output)
end