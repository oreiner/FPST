function [posterior, out] = demo_QlearningAsymetric (data, asym, SigmaPhi, SigmaX0, muX0, muTheta, muPhi)
% // VBA toolbox //////////////////////////////////////////////////////////
%
...
if not(exist('muX0','var'))
   muX0 = 0.5; 
end
if not(exist('SigmaX0','var'))
   SigmaX0 = 0.01; 
end
options.priors.muX0 = muX0 * ones (dim.n, 1);
options.priors.SigmaX0 = SigmaX0 * eye (dim.n);

options.priors.SigmaTheta = diag([0.1 0.1*asym]);
%options.priors.muThera  = [1 ; 1];  %default I think. doesn't need to be 

if not(exist('SigmaPhi','var'))
   SigmaPhi = 1; 
end

options.priors.muPhi = log(13.5);
options.priors.SigmaPhi = 0.1*SigmaPhi;
if exist('muTheta') && exist ('muPhi') %use learn phase data in test phase when calculating seperatly 
